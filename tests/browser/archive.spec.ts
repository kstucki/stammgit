...
test('read-only role and unauthenticated requests retain server-side gates', async ({ page, request }) => {
  ...
  await expect(page.locator('#personDialog').getByRole('link', { name: 'In Stammbaum anzeigen', exact: true })).toBeVisible();
...
for (const role of ['admin', 'reader']) {
  test(`person tree action opens the centered family and closes dialogs for ${role}`, async ({ page, isMobile }) => {
    ...
    async function showFamily(id: string) {
      ...
      const action = dialog.getByRole('link', { name: 'In Stammbaum anzeigen', exact: true });
    ...
    await page.locator('#personDialog').getByRole('button', { name: 'Test Bruno', exact: true }).click();
